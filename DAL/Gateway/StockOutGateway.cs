...
namespace StockManagementSystem.DAL.Gateway
{
    public class StockOutGateway : CommonGateway
    {
        ...
        public int SaveStockOut( List<StockOut> stockout)
        {
            int rowAffected = 0;
            foreach (var outs in stockout)
            {
                Query = "Insert into StockOut_tb (Item,Company,Quantity,StockOutType) values (@Item,@Company,@Quantity,@StockOutType) ";

                Command = new SqlCommand(Query, Connection);
                Connection.Open();

                Command.Parameters.AddWithValue("Item", outs.Item);
                Command.Parameters.AddWithValue("Company", outs.Company);
                Command.Parameters.AddWithValue("Quantity", outs.Quantity);
                Command.Parameters.AddWithValue("StockOutType", outs.Type);
